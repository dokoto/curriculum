...
class CpuUsage extends EventsEmiter {
    ...
    start() {
        ...
        console.debug('[CPU-USAGE][START] Total workers %d', this._totalWorkers);
        for (const worker in cluster.workers) {
    ...
    _handleCpuUsage(worker, index, err, stat) {
        this._totalWorkers--;
        if (!err) {
            console.debug('[CPU-USAGE][%d][%d] Process %d at cpu %d', this._totalWorkers, index, worker.process.pid, stat.cpu);
            this.workers[worker.process.pid] = {
                'stat': stat,
                'process': worker.process
            };
        } else {
            console.debug('[CPU-USAGE][%d][%d][%d] %s', this._totalWorkers, index, worker.process.pid, JSON.stringify(err));
        }
        if (this._totalWorkers === 0) {
            console.debug('[CPU-USAGE] Setting new timer at %d', this.timeInterval);
    ...
    getLowerLoadWorker() {
        ...
        for (const worker in this.workers) {
            console.debug('[CPU-USAGE][MEASURE-CPU] Pid: %d cpu load: %d', this.workers[worker].process.pid, this.workers[worker].stat.cpu);
            workerLowLoad = (workerLowLoad.stat.cpu > this.workers[worker].stat.cpu) ? this.workers[worker] : workerLowLoad;
        }

        console.debug('[CPU-USAGE] Lower worker is %d at cpu %d', workerLowLoad.process.pid, workerLowLoad.stat.cpu);
